...
(after! markdown-mode
  ...
  (custom-set-faces!
    ...
    '(markdown-hr-face             :foreground "#1e1e1e" :strike-through "#3a3a3a")
    ;; Lista
  ...
  (defun nb/markdown-warm-theme ()
    ...
    (face-remap-add-relative 'default
      :background "#1e1e1e" :foreground "#d0d0d0" :family "Charter" :height 210)
    ;; Fringe sigue el fondo
    (face-remap-add-relative 'fringe :background "#1e1e1e")
    ;; Line numbers casi invisibles
    (face-remap-add-relative 'line-number :foreground "#3a3a3a" :background "#1e1e1e")
    (face-remap-add-relative 'line-number-current-line :foreground "#606060" :background "#1e1e1e")
    ;; Region/selección sutil
    (face-remap-add-relative 'region :background "#2a2a2a")
    ;; Cursor azul clásico
    (face-remap-add-relative 'cursor :background "#61afef")
    (setq-local evil-normal-state-cursor '(box "#61afef"))
    (setq-local evil-insert-state-cursor '(bar "#61afef"))
    (setq-local evil-visual-state-cursor '(hollow "#61afef"))
...
Ignora líneas de tabla — valign maneja su display."
  ...
  (defun nb/update-table-overlays ()
    ...
    (let ((bg-alt "#252525")) ;; tabla: bg-alt Stone
      (save-excursion
        (goto-char (point-min))
