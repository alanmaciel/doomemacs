...
(after! markdown-mode
  ...
  ;; --- Warm/Sepia theme para markdown ---
  ;; Paleta: fondo marrón oscuro cálido, texto crema, acentos ámbar/terracota
  (custom-set-faces!
    ;; Links
    '(markdown-link-face           :foreground "#D4A055" :underline t)
    '(markdown-url-face            :foreground "#6B5B4E" :height 0.85)
    ;; Strikethrough
    ...
    ;; Code inline
    '(markdown-inline-code-face    :foreground "#E8C47C" :background "#2A2017" :family "CaskaydiaMono Nerd Font" :weight normal)
    ;; Code blocks
    '(markdown-pre-face            :foreground "#C8B08A" :background "#1E1810" :extend t :family "CaskaydiaMono Nerd Font" :weight normal)
    '(markdown-code-face           :foreground "#C8B08A" :background "#1E1810" :extend t :family "CaskaydiaMono Nerd Font" :weight normal)
    ...
    '(markdown-header-face-2 :height 1.4  :foreground "#E8A840" :weight extra-bold :inherit markdown-header-face)
    '(markdown-header-face-3 :height 1.2  :foreground "#D4825A" :weight extra-bold :inherit markdown-header-face)
    '(markdown-header-face-4 :height 1.15 :foreground "#C46B50" :weight bold       :inherit markdown-header-face)
    '(markdown-header-face-5 :height 1.1  :foreground "#B07A6B" :weight bold       :inherit markdown-header-face)
    '(markdown-header-face-6 :height 1.05 :foreground "#9B8578" :weight semi-bold  :inherit markdown-header-face)
    ;; Blockquotes
    '(markdown-blockquote-face     :foreground "#A89580" :slant italic)
    ;; HR
    '(markdown-hr-face             :foreground "#5A4A3A" :strike-through "#5A4A3A")
    ;; Lista
    '(markdown-list-face           :foreground "#D4A055"))

  ;; Fondo cálido + fuente serif para prosa (solo en buffers markdown)
  (defun nb/markdown-warm-theme ()
    "Aplica tema warm/sepia al buffer markdown."
    ;; Fondo marrón oscuro cálido
    (face-remap-add-relative 'default
      :background "#1C1612" :foreground "#D5C4A1" :family "Charter" :height 150)
    ;; Fringe sigue el fondo
    (face-remap-add-relative 'fringe :background "#1C1612")
    ;; Line numbers más sutiles
    (face-remap-add-relative 'line-number :foreground "#4A3C2E" :background "#1C1612")
    (face-remap-add-relative 'line-number-current-line :foreground "#8B7355" :background "#1C1612")
    ;; Region/selección cálida
    (face-remap-add-relative 'region :background "#3A2A1E"))
...
Ignora líneas de tabla — valign maneja su display."
  ...
  (defun nb/update-table-overlays ()
    ...
    (let ((bg-alt "#241C14")) ;; tabla: ligeramente más oscuro que el fondo sepia
      (save-excursion
        (goto-char (point-min))
