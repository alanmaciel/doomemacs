...
(after! markdown-mode
  ...
  (custom-set-faces!
    ...
    '(markdown-hr-face             :foreground "#4A3728" :strike-through "#C4A265")
    ;; Lista
    '(markdown-list-face           :foreground "#D4A055")
    ;; Tablas
    '(markdown-table-face          :foreground "#F3EDE4" :background "#4A3728" :family "Iosevka Nerd Font" :weight light :height 140))
  ...
  (defun nb/markdown-warm-theme ()
    ...
    (face-remap-add-relative 'fringe :background "#4A3728")
    ;; Line numbers sutiles
    (face-remap-add-relative 'line-number :foreground "#6B5744" :background "#4A3728")
    (face-remap-add-relative 'line-number-current-line :foreground "#A89070" :background "#4A3728")
...
Ignora líneas de tabla — valign maneja su display."
  ...
  (defun nb/update-table-overlays ()
    ...
    (let ((bg-alt "#4A3728")) ;; tabla: mismo fondo que el resto del buffer
      (save-excursion
        (goto-char (point-min))
