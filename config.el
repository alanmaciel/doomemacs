...
(after! markdown-mode
  ...
  ;; --- Spacemacs Light theme para markdown ---
  ;; Paleta: fondo crema cálido, texto gris-púrpura, headings coloridos
  (custom-set-faces!
    ;; Links
    '(markdown-link-face           :foreground "#3a81c3" :underline t)
    '(markdown-url-face            :foreground "#a094a2" :height 0.85)
    ;; Strikethrough
    ...
    '(markdown-header-face-2 :height 1.4  :foreground "#2d9574" :weight extra-bold :inherit markdown-header-face)
    '(markdown-header-face-3 :height 1.2  :foreground "#67b11d" :weight extra-bold :inherit markdown-header-face)
    '(markdown-header-face-4 :height 1.15 :foreground "#b1951d" :weight bold       :inherit markdown-header-face)
    '(markdown-header-face-5 :height 1.1  :foreground "#6c3163" :weight bold       :inherit markdown-header-face)
    '(markdown-header-face-6 :height 1.05 :foreground "#715ab1" :weight semi-bold  :inherit markdown-header-face)
    ;; Blockquotes
    '(markdown-blockquote-face     :foreground "#2aa1ae" :slant italic)
    ;; HR
    '(markdown-hr-face             :foreground "#b3b9be" :strike-through "#b3b9be")
    ;; Lista
    '(markdown-list-face           :foreground "#6c3163"))

  ;; Fondo claro + fuente serif para prosa (solo en buffers markdown)
  (defun nb/markdown-warm-theme ()
    "Aplica tema spacemacs-light al buffer markdown."
    ;; Fondo crema cálido
    (face-remap-add-relative 'default
      :background "#fbf8ef" :foreground "#655370" :family "Charter" :height 150)
    ;; Fringe sigue el fondo
    (face-remap-add-relative 'fringe :background "#fbf8ef")
    ;; Line numbers sutiles
    (face-remap-add-relative 'line-number :foreground "#a8a8bf" :background "#fbf8ef")
    (face-remap-add-relative 'line-number-current-line :foreground "#655370" :background "#fbf8ef")
    ;; Region/selección
...
Ignora líneas de tabla — valign maneja su display."
  ...
  (defun nb/update-table-overlays ()
    ...
    (let ((bg-alt "#efeae9")) ;; tabla: ligeramente más oscuro que el fondo crema
      (save-excursion
        (goto-char (point-min))
