...
- [[#org][Org]]
  ...
  - [[#toc-org][TOC Org]]
- [[#document-and-image-preview][Document and image preview]]
  - [[#images][Images]]
  - [[#pdf][PDF]]
- [[#ai][AI]]
...
** PDF
=pdf-tools= comes from Doom's =:tools pdf= module. The =epdfinfo= binary is
provided by Nix (=emacsPackages.pdf-tools= in configuration.nix), so Emacs must
never build it under =~/.config/emacs=.
