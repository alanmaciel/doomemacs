...
** Warnings level
Suppress the noisy =:warning=-level chatter at startup. Note that this also
hides plain =(warn …)= calls (they emit at =:warning=), so the config guards
below use =(display-warning 'doom-config … :emergency)= to stay visible.
#+begin_src emacs-lisp
...
Show images inline when opening a markdown buffer, sized for the small portrait
screen. The hook itself is registered by =nb/markdown-mode-setup= above, which
guarantees this runs *after* =nb/markdown-warm-theme= and olivetti have fixed the
window width, so images are measured against the definitive width.
...
** Images
Open jpg/gif/svg/png/webp directly in =image-mode=, scaled to the window width.
Emacs is built with native jpeg, gif, svg, png and webp support, so no external
...
** PDF
=pdf-tools= viene del módulo =:tools pdf= de Doom. El binario =epdfinfo= lo
provee Nix (=emacsPackages.pdf-tools= en configuration.nix), así que Emacs nunca
debe compilarlo en =~/.config/emacs=.
